...
#ifndef VERILATED_VCLOCK_TB_H_
#define VERILATED_VCLOCK_TB_H_  // guard

#include "verilated.h"

class Vclock_tb__Syms;
class Vclock_tb___024root;
class Vclock_tb___024unit;


// This class is the main interface to the Verilated model
class alignas(VL_CACHE_LINE_BYTES) Vclock_tb VL_NOT_FINAL : public VerilatedModel {
    friend class Vclock_tb__Syms;
  private:
    // Symbol table holding complete model state (owned by this class)
    Vclock_tb__Syms* const vlSymsp;
    // Evaluation loop
  ...
  public:
    ...
    Vclock_tb___024unit* const __PVT____024unit;

    // Root instance pointer to allow access to model internals,
    // including inlined /* verilator public_flat_* */ items.
    Vclock_tb___024root* const rootp;
    ...
    explicit Vclock_tb(VerilatedContext* contextp, const char* name = "TOP");
    explicit Vclock_tb(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~Vclock_tb();
  private:
    VL_UNCOPYABLE(Vclock_tb);  ///< Copying not allowed
