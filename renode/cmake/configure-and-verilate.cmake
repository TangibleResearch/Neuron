# Vendored from antmicro/renode-verilator-integration
# (cmake/configure-and-verilate.cmake), MIT licensed — see LICENSE in this
# directory. This is a generic Verilator+Renode-IntegrationLibrary build
# helper meant to be copied per-project; it is not Neuron-specific. One
# deliberate local change from upstream: see the PREFIX note near the
# verilate() calls below.
...
if(${CMAKE_CXX_COMPILER_ID} STREQUAL "AppleClang")
  # MODIFIED from upstream: this used to pin C++11 ("has to be at least
  # c++11"), but the Verilator version in use here (5.052) generates code
  # requiring C++14 (auto-deduced returns, std::exchange, etc.) — pinning
  # to 11 makes verilated.h itself fail to compile. Bump to 17 as a safe
  # modern floor; there's nothing in this project relying on old-standard
  # quirks.
  set_property(TARGET Vtop PROPERTY CXX_STANDARD 17)
  set_property(TARGET libVtop PROPERTY CXX_STANDARD 17)
...
# MODIFIED from upstream: added `PREFIX ${TOP_MODULE}` — with multiple SV
# source files, Verilator otherwise names the generated top-level class
# after the *first* source file rather than TOP_MODULE, which breaks any
# `#include "V<top_module>.h"` in project code (see renode/README.md).
verilate(Vtop SOURCES ${VTOP} TOP_MODULE ${TOP_MODULE} PREFIX V${TOP_MODULE} VERILATOR_ARGS ${FINAL_EXEC_VERI_ARGS})
verilate(libVtop SOURCES ${VTOP} TOP_MODULE ${TOP_MODULE} PREFIX V${TOP_MODULE} VERILATOR_ARGS ${FINAL_LIB_VERI_ARGS})
